...
class CodeGenerator extends GolampiBaseVisitor
{
    ...
    */
    public function visitPostfix($ctx)
    {
        ...
        // Iterar children en orden para respetar el encadenamiento
        // primary ya fue visitado, ahora procesamos los sufijos
        ...
        if ($ctx->exprList() !== null) {
            ...
            foreach ($exprs as $i => $expr) {
                ...
                switch ($typeHint) {
                    ...
                    case "nil":
                        $nilLabel = $this->addString("<nil>");
                        $this->requireHelper("__strlen");
                        $this->emit("    adrp    {$reg}, {$nilLabel}");
                        $this->emit("    add     {$reg}, {$reg}, :lo12:{$nilLabel}");
                        $this->emit("    mov     x0, {$reg}");
                        $this->emit("    bl      __strlen");
                        $this->emit("    mov     x1, x0");
                        $this->emit("    mov     x0, {$reg}");
